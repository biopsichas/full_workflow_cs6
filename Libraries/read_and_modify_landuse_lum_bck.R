...
## cn2
lum$cn2[which(substr(lum$name,1,1)=='agrl')] <- 'rc_strow_g'
...
## cons_prac
lum$cons_prac[which(substr(lum$name,1,1)=='agrl')] <- 'up_down_slope'
lum$cons_prac[which(substr(lum$name,1,2)=='fr')] <- 'up_down_slope'
lum$cons_prac[which(substr(lum$name,1,2)=='fe')] <- 'up_down_slope'
lum$cons_prac[which(substr(lum$name,1,4)=='past')] <- 'up_down_slope'
...
lum$cons_prac[which(substr(lum$name,1,4)=='grap')] <- 'up_down_slope'
lum %>% filter(cons_prac == "null") %>% select(name) %>% unique()

## ov_mann
lum$ov_mann[which(substr(lum$name,1,1)=='agrl')] <- 'convtill_nores'
